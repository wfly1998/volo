...
impl<S, L> Server<S, L> {
    ...
    pub async fn run_with_builder_cfg<MI, F>(
        self,
        mk_incoming: MI,
        builder_cfg: F,
    ) -> Result<(), BoxError>
    where
        ...
        L::Service: Service<ServerContext, BodyIncoming, Response = Response, Error = Infallible>
            ...
            + 'static,
        MI: MakeIncoming,
        F: FnOnce(http1::Builder) -> http1::Builder + Clone + Send + Sync + 'static,
    {
        // init server
        let service = Arc::new(self.layer.layer(self.service));
        // TODO(lyf1999): type annotation is needed here, figure out why
            ...
            loop {
                ...
                match incoming.accept().await {
                    Ok(Some(conn)) => {
                        ...
                        tokio::task::spawn(handle_conn_with_builder_cfg(
                            conn,
                            service.clone(),
                            builder_cfg.clone(),
                            stat_tracer.clone(),
...
async fn handle_conn_with_builder_cfg<S, F>(
    conn: Conn,
    service: S,
    builder_cfg: F,
    stat_tracer: Arc<[TraceFn]>,
    ...
    F: FnOnce(http1::Builder) -> http1::Builder + Clone + Send + Sync + 'static,
{
    ...
    let mut http_conn = builder_cfg(http1::Builder::new()).serve_connection(
        TokioIo::new(conn),
        hyper::service::service_fn(|req| {
            serve(service.clone(), peer.clone(), stat_tracer.clone(), req)
        }),
